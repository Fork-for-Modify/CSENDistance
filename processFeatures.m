...
param.DicDesign = '2D';
inputData = strcat('./data/features/kitti-data/features_max_', param.modelName, '.mat');
load(inputData)
...
for k = 1:nuR
	...
	% figure
	figure,
	scatter(y_trues(k, :), y_preds(k, :), 3, 'filled'), 
	title(strcat('Run-',num2str(k), ': Collaborative Filtering, MSE: ', ...
		num2str(sum((y_trues(k, :)- y_preds(k, :)).^2)/length(y_trues(k, :)))))
